...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.sp = tf.placeholder(dtype=tf.uint8, shape=[None, img_height, img_width, nchannels * self.config.state_history], name='nextstate')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            flattened = tf.layers.flatten(state)
            out = tf.layers.dense(flattened, num_actions)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        for i in range(len(target_q_vars)):
            update_op_list.append(tf.assign(target_q_vars[i], q_vars[i]))
        self.update_target_op = tf.group(*update_op_list)
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_samp = self.r + (1 - tf.cast(self.done_mask, tf.float32)) * self.config.gamma * tf.reduce_max(target_q, axis=1)
    ...
    def add_optimizer_op(self, scope):
        ...
        grads_vars = optimizer.compute_gradients(self.loss, scope_vars)
        if self.config.grad_clip:
            clipped_grads_vars = list()
            for g, v in grads_vars:
                clipped_grads_vars.append((tf.clip_by_norm(g, self.config.clip_val), v))
        self.train_op = optimizer.apply_gradients(clipped_grads_vars)
        self.grad_norm = tf.global_norm([g for g, _ in clipped_grads_vars])
