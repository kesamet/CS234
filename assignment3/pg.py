...
def build_mlp(
  ...
  with tf.variable_scope(scope):
    ...
    for _ in range(n_layers):
      out = tf.layers.dense(out, units=size, activation='relu')
...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    self.observation_placeholder = tf.placeholder(
      dtype=tf.float32, shape=[None, self.observation_dim], name='observation')
    if self.discrete:
      self.action_placeholder = tf.placeholder(
        dtype=tf.int32, shape=[None], name='action')
    else:
      self.action_placeholder = tf.placeholder(
        dtype=tf.float32, shape=[None, self.action_dim], name='action')
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      ...
      self.sampled_action = tf.random.normal(
        shape=tf.shape(action_means), means=action_means, stddev=tf.exp(log_std))
      self.logprob = tf.contrib.distributions.MultivariateNormalDiag(
        loc=action_means, scale_diag=tf.exp(log_std)
  ...
  def add_baseline_op(self, scope = "baseline"):
    ...
    self.baseline = tf.squeeze(build_mlp(
      ...
      self.config.n_layers, self.config.layer_size))
    self.baseline_target_placeholder = tf.placeholder(dtype=tf.float32, shape=[None])
    baseline_loss = tf.losses.mean_squared_error(
      self.baseline_target_placeholder, self.baseline)
    self.update_baseline_op = tf.train.AdamOptimizer(
      learning_rate=self.lr).minimize(baseline_loss)
    #######################################################
    #########          END YOUR CODE.          ############
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      for i in range(T):
        returns.append(
          (self.config.gamma ** np.arange(T-i) * np.array(rewards[i:])).sum()
        )
      #######################################################
      #########          END YOUR CODE.          ############
